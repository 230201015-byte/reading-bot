...
async def choose_action(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "📊 Google Sheets-ті қарау":
        explanation = (
            ...
            "• date — күн (2026)\n"
            "• pages — бет саны\n\n"
            "Кестені төмендегі батырма арқылы ашуға болады."
        )

        sheet_keyboard = ReplyKeyboardMarkup(
            [
                ["🔗 Google Sheets-ке өту"],
                ["⬅️ Бас менюге қайту"]
            ],
            resize_keyboard=True
        )
    ...
    if text == "🔗 Google Sheets-ке өту":
        await update.message.reply_text(
            "📎 Кесте сілтемесі:\n"
            "https://docs.google.com/spreadsheets/d/1joO33x2UdcGWJTk8F--GCpPpmP-MnyB0YfJpatlcu2k/edit"
        )
        return SELECT_ACTION
...
async def choose_date(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not text.isdigit() or not (1 <= int(text) <= 31):
        await update.message.reply_text(
            "❌ Күнді 1 мен 31 аралығында таңда."
        )
        return SELECT_DATE
...
def main():
    ...
    conversation = ConversationHandler(
        ...
        states={
            ...
            SELECT_PERSON: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, choose_person)
            ],
            SELECT_DATE: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, choose_date)
            ],
            ENTER_PAGES: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, enter_pages)
            ],
        },
        fallbacks=[],
    )
